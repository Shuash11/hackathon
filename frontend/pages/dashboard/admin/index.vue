...
const { data: analytics, status, error, refresh } = await useAsyncData('admin-analytics', () => getAnalytics())
...
<template>
  <div>
    <header class="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p class="text-[10px] font-bold uppercase tracking-[0.22em] text-atelier-clay">Admin</p>
        <h1 class="mt-2 font-display text-4xl leading-tight sm:text-5xl">Business overview</h1>
    ...
    <template v-else-if="loading || !analytics">
    ...
    <template v-else>
      <section aria-label="Business summary" class="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Products" :value="analytics.total_products" detail="Across four collections" icon="products" />
        <StatCard label="All orders" :value="analytics.total_orders" :detail="`${analytics.orders_this_month} received this month`" icon="orders" />
        <StatCard label="Total revenue" :value="formatCompactCurrency(analytics.total_revenue)" detail="Excludes cancelled orders" icon="revenue" accent />
        <StatCard label="Monthly revenue" :value="formatCompactCurrency(analytics.revenue_this_month)" detail="Current calendar month" icon="trend" />
      </section>
      ...
      <section aria-labelledby="collections-heading" class="mt-10">
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <CategoryCard v-for="category in analytics.orders_by_category" :key="category.slug" :category="{ id: 0, name: category.name, slug: category.slug, product_count: 0 }" />
        </div>
      </section>

      <section aria-label="Sales analysis" class="mt-8 grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <CategoryBreakdownChart :data="analytics.orders_by_category" />
        <BestSellersList :products="analytics.best_sellers" />
      </section>

      <section aria-label="Order activity" class="mt-8 grid gap-6 2xl:grid-cols-[1fr_19rem]">
        <RecentOrdersList :orders="analytics.recent_orders" />
        <article class="overflow-hidden rounded-2xl border border-atelier-line bg-atelier-ink p-5 text-white sm:p-6">
          ...
          <ul v-if="analytics.low_stock.length" class="mt-6 divide-y divide-white/10">
            <li v-for="product in analytics.low_stock" :key="product.id" class="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
              <div class="min-w-0">
          ...
          <div class="mt-6 border-t border-white/10 pt-5">
            ...
            <p class="mt-2 font-display text-2xl">{{ formatCurrency(analytics.total_revenue) }}</p>
          </div>
        </article>
      </section>
    </template>
